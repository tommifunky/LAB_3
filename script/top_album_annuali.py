...
output_path = os.path.join("..", "Dati_lavorati", "top_album_annuali.json")
...
output = {}
for year, albums in per_anno.items():
    output[str(year)] = [
        {"album": k[0], "artist": k[1], "plays": v["plays"], "ms_played": v["ms_played"]}
        for k, v in sorted(albums.items(), key=lambda x: x[1]["ms_played"], reverse=True)
    ]
